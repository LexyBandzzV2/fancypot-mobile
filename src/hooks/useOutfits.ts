...
import { listOutfits, deleteOutfit as apiDelete, type Outfit } from '@/lib/api';
...
export function useOutfits() {
  ...
  const save = useCallback(
    async (data: Partial<Outfit>) => {
      ...
      // persistOutfitImage uploads a base64 `data:` image to storage first (a
      // data URL can't be signed, so the Saved tile would render blank), then
      // inserts the row. Shared with the try-on background saver.
      await persistOutfitImage(user.id, data);
      await load();
    },
    [user, load],
  );
